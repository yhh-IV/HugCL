...
import sys
sys.path.append("C:/Users/Haohan/LSTCRL/motion_planning/")
from tools import *
...
class Controller(object):
    ...
    def get_control(self, ego_state, trajectory, target_speed):

        acc = self._lon_controller.run_step(target_speed, ego_state.v)
        steering = self._lat_controller.run_step(ego_state, trajectory)
...
class PurePuesuitController(object):
    ...
    def run_step(self, ego_state, trajectory):
        """
        Execute one step of lateral control to steer the vehicle towards a certain waypoin.
        """

        current_speed = ego_state.v
        control_point = self._control_point(ego_state, trajectory, current_speed)
        if len(control_point) < 2:
            return 0.0
        return self._purepersuit_control(control_point, ego_state)

    def _control_point(self, ego_state, trajectory, current_speed, resolution = 0.1):
